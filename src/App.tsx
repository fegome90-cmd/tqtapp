...
import { Play, ChevronLeft, Home, MessageSquare, Star, User, Mic, AlertTriangle, Wind, Droplets, Activity, Move, Users, Coffee, Heart, Volume2 } from 'lucide-react';
...
const Icons: Record<string, React.FC<any>> = {
  AlertTriangle, Wind, Droplets, Activity, Move, Users, Coffee, Heart
};

export default function App() {
  const[currentTab, setCurrentTab] = useState<'home' | 'talk' | 'favs' | 'profile'>('home');
  const [activeCategory, setActiveCategory] = useState<CategoryId | null>(null);
  const [favorites, setFavorites] = useState<string[]>(['p1', 'p16', 'p17', 'p18']); // Default favs
  ...
  return (
    <div className="w-full max-w-2xl mx-auto h-screen sm:h-[95vh] sm:mt-[2.5vh] sm:rounded-[2.5rem] bg-slate-50 flex flex-col font-sans relative sm:shadow-2xl overflow-hidden sm:border-[8px] sm:border-slate-800">
      ...
      <main className="flex-1 overflow-y-auto pb-24 relative">
            ...
            <div className="p-5 space-y-6">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-2xl font-bold tracking-wide uppercase">Necesito Ayuda</span>
                </div>
              ...
              <div className="space-y-4">
                <h2 className="text-sm font-semibold text-slate-500 uppercase tracking-widest ml-1">Categorías Frecuentes</h2>
                  ...
                  {CATEGORIES.map(cat => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-lg font-semibold text-slate-800">{cat.title}</span>
                      </button>
                    );
                  })}
                </div>
        ...
        {activeCategory && currentTab === 'home' && (
          <div className="transition-all duration-300 transform translate-x-0">
            <TopBar title={CATEGORIES.find(c => c.id === activeCategory)?.title || 'Frases'} showBack />
            <div className="p-5 space-y-4">
              {MOCK_PHRASES.filter(p => p.categoryId === activeCategory).map(phrase => (
                <div key={phrase.id} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex items-stretch min-h-[90px]">
                  <button
                  ...
                  >
                    <div className={cn(
                      ...
                      playingId === phrase.id ? "bg-blue-500 scale-125 shadow-[0_0_12px_rgba(59,130,246,0.6)]" : "bg-slate-200"
                    )} />
                    <span className="text-xl font-medium text-slate-800 leading-tight">{phrase.text}</span>
                  </button>
                  <button
                  ...
                  >
                    <Star className={cn("w-8 h-8 transition-all", favorites.includes(phrase.id) ? "fill-yellow-400 text-yellow-400" : "text-slate-300")} />
                  </button>
                </div>
              ))}
              {MOCK_PHRASES.filter(p => p.categoryId === activeCategory).length === 0 && (
            ...
            <div className="p-5 flex-1 flex flex-col gap-5">
              <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 flex-1 relative flex flex-col min-h-[250px]">
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 pt-4 no-scrollbar border-t border-slate-50">
                  {['Sí', 'No', 'Gracias', 'Por favor', 'Me duele'].map(sug => (
                    <button 
                      key={sug} 
                      onClick={() => setFreeText(prev => prev ? `${prev} ${sug}` : sug)}
              ...
              >
                ...
                <span className="text-xl font-semibold tracking-wide">Reproducir Mensaje</span>
              </button>
              ...
              <div className="mt-2 p-5 rounded-3xl border-2 border-dashed border-slate-200 bg-white/50 flex items-center gap-4 opacity-80">
                ...
                <div>
                  <h4 className="font-bold text-slate-800 text-lg">Dictado por voz y clonación</h4>
                  <p className="text-sm text-slate-500 font-medium">El input predictivo de audio está agendado para la Fase 2.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="p-5 space-y-4">
              {favorites.length === 0 ? (
                <div className="text-center py-24 px-6">
                  ...
                  <p className="text-slate-500 font-medium text-xl">Aún no tienes frases favoritas.</p>
                  <p className="text-slate-400 text-base mt-2">Marca la estrella en cualquier frase frecuente para guardarla aquí.</p>
                </div>
              ) : (
                MOCK_PHRASES.filter(p => favorites.includes(p.id)).map(phrase => (
                  <div key={phrase.id} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden flex items-stretch min-h-[90px]">
                    <button 
                      onClick={() => handlePlay(phrase.id)} 
                      className="flex-1 p-6 text-left active:bg-slate-50 flex items-center gap-5 transition-colors"
                    >
                      <div className={cn("w-4 h-4 rounded-full transition-all", playingId === phrase.id ? "bg-blue-500 scale-125" : "bg-slate-200")} />
                      <span className="text-xl font-medium text-slate-800">{phrase.text}</span>
                    </button>
                    <button 
                      onClick={() => toggleFavorite(phrase.id)} 
          ...
          <div className="transition-opacity duration-300">
            ...
            <div className="p-5 space-y-8">
              ...
              <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">Mario Rojas</h2>
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-widest ml-2">Accesibilidad</h3>
              ...
              <div className="space-y-3">
                <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-widest ml-2">Preparación Quirúrgica</h3>
                <div className="bg-gradient-to-br from-blue-600 to-indigo-800 p-7 rounded-[2rem] shadow-lg relative overflow-hidden">
                  ...
                  <h4 className="text-2xl font-bold text-white mb-2 relative z-10">Mi Banco de Voz</h4>
                  <p className="text-base text-blue-100 mb-6 relative z-10 leading-relaxed">
                    Graba tu voz antes de la laringectomía para que podamos sintetizarla y la app suene exactamente como tú en el futuro.
                  </p>
                  
                  <button className="w-full bg-white text-indigo-700 font-bold text-lg py-4 rounded-2xl shadow-sm hover:bg-blue-50 active:scale-[0.98] transition-all relative z-10">
                    Continuar Grabando
                  </button>
                </div>
              </div>
        ...
        {[
          ...
          >
            <item.icon className={cn(
              ...
              currentTab === item.id ? "text-blue-600 scale-110" : "text-slate-400"
            )} />
            <span className={cn(
      ...
      {/* Estilos inyectados localmente para utilidades de la maqueta */}
